...
async function storeMessage(chatRoom, msg) {
  try {
    updatedMsg = await messageModel.updateOne(
      { room: chatRoom },
      { $push: { messages: msg } }
    );
  } catch (error) {
...
io.on("connection", (socket) => {
  ...
  socket.on("send_message", async (msg) => {
    // receiving the messages that coming from frontend
...
if ("production" === "production") {
  ...
  console.log(
    path.resolve(__dirname1, "frontend", "index.html")
    // path.resolve(__dirname1, "..", "frontend", "dist", "frontend", "index.html")
  );
  // backend\index.html
  app.get("*", (req, res) =>
    res.sendFile(
